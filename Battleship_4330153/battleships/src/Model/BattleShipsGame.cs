...
public class BattleShipsGame
{
	...
	public AttackResult Shoot (int row, int col)
	{
		...
		int _killRemain = (Enum.GetValues (typeof (ShipName)).Length - 1) - (GameController.HumanPlayer.PlayerGrid.ShipsKilled) - 1;

		newAttack = Player.Shoot (row, col);

		//if (newAttack.Value == ResultOfAttack.Destroyed) {
		//	Audio.StopSoundEffect (GameResources.GameSound ("destroyed"));
		//	Audio.PlaySoundEffect (GameResources.GameSound ("destroyed"));
		//}
